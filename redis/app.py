    ...
    try:
        ...
        if cache.sismember("active_meetings", f"meeting:{meeting_signature}"):
            ...
            if cache.sismember(f"meeting:{meeting_signature}:joined", user_id):
                ...
                raise Exception(
                    f"user with email {email} hasn't joined the meeting is not allowed into the meeting!")
        else:
            raise Exception(
                f"meeting with signature {meeting_signature} not active!")
    except Exception as e:
...
def show_chat(meeting_signature):
    try:
        if cache.sismember("active_meetings", f"meeting:{meeting_signature}"):
            # if a meeting has an audience, then the email should be in the audience set
            result = cache.lrange(
                f"meeting:{meeting_signature}:messages", 0, -1)
...
def show_user_chat(meeting_signature, userID):
    try:
        if cache.exists(f"message:{meeting_signature}:{userID}"):
            # if a meeting has an audience, then the email should be in the audience set
            result = cache.lrange(
                f"message:{meeting_signature}:{userID}", 0, -1)
            return utils.format_results_dict(result)
        else:
            raise Exception(
                f"user has not posted anything in meeting {meeting_signature} or the meeting is inactive!")
    except Exception as e:
        logging.error(f"Error: {str(e)}")
        return render_template('index.html', posts=[f"Error:{str(e)}"])
